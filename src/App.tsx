...
export default function App() {
  ...
  // 2. FUNÇÃO DE SUBMIT CORRIGIDA (SEM ERROS DE FILTRAGEM OU ORTOGRAFIA)
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    // Ir buscar o link do Stripe correspondente ao primeiro item selecionado
    ...
    try {
      // Grava a reserva no Firebase dentro de um bloco isolado para não trancar o pagamento
      try {
        await addDoc(collection(db, 'bookings'), bookingData);
      } catch (firebaseErr) {
        console.error("Aviso do Firebase (Ignorado para permitir pagamento):", firebaseErr);
      }

      // Limpa os estados locais e redireciona de forma limpa para o Stripe
      setCart([]);
      setIsCartOpen(false);
      
      // Redirecionamento nativo e seguro do navegador
        ...
        <div className="fixed inset-0 bg-neutral-900/60 z-50 flex justify-end backdrop-blur-sm">
          <div className="bg-white w-full max-w-md h-full flex flex-col justify-between shadow-2xl overflow-y-auto p-6">
            <div>
              ...
              {/* 3. FORMULÁRIO CONFIGURADO CORRETAMENTE COM EVENTO ONSUBMIT */}
              <form onSubmit={handleCheckoutSubmit} className="flex flex-col gap-3.5">
                ...
                <textarea placeholder={lang === 'pt' ? 'Notas adicionais (ex: código da porta, allergies...)' : 'Additional notes (e.g. door code, allergies...)'} value={deliveryNotes} onChange={(e) => setDeliveryNotes(e.target.value)} className="border p-2.5 rounded-xl text-xs w-full h-16 resize-none focus:outline-amber-600" />
                
                {submitError && <p className="text-red-500 text-xs font-bold">{submitError}</p>}

                {/* BOTÃO LIMPO DE EVENTOS INSTÁVEIS, CONFIANDO APENAS NO SUBMIT DO FORMULÁRIO */}
