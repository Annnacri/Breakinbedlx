...
export default function App() {
  ...
  // FUNÇÃO DE SUBMISSÃO ESTRUTURADA PARA AS REGRAS DO FIREBASE
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    // Gerar um ID compatível com a tua regra isValidId(bookingId)
    const customBookingId = `bk-${Date.now()}`;

    // Mapear os dados garantindo correspondência total com o teu esquema de segurança JSON
    const bookingData = {
      id: customBookingId,
    ...
    try {
      // Usar setDoc para forçar o envio do ID que valida na tua regra do Firestore
      await setDoc(doc(db, 'bookings', customBookingId), bookingData);

      setCart([]);
      setIsCartOpen(false);
      
      // Avança de forma limpa para o link correto do Stripe
      window.location.assign(stripeCheckoutUrl);
      
    } catch (err) {
      console.error("Erro na validação de regras do Firebase:", err);
      setSubmitError(lang === 'pt' 
        ? 'Erro na submissão de dados de agendamento.' 
        : 'Error submitting booking reservation data.');
    } finally {
      setIsSubmitting(false);
    }
  };
